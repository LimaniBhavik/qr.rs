...
fn vcard_while_matches(s: &str, out: &mut String) {
    let mut last_pos = 0;
...
fn vcard_iter_table(s: &str, out: &mut String) {
    let bytes = s.as_bytes();
    let mut last_pos = 0;
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("vcard_methods");

    for s in &test_strings {
        group.bench_function(format!("while_matches_{}", s), |b| {
            b.iter(|| {
                let mut out = String::with_capacity(128);
                vcard_while_matches(black_box(s), &mut out);
                black_box(out);
            })
        });
        ...
        group.bench_function(format!("iter_table_{}", s), |b| {
            b.iter(|| {
                let mut out = String::with_capacity(128);
                vcard_iter_table(black_box(s), &mut out);
                black_box(out);
            })
        });
    }

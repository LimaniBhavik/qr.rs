...
fn escape_vcard_while_matches(s: &str, out: &mut String) {
    let mut last_pos = 0;
...
fn escape_vcard_while_table(s: &str, out: &mut String) {
    let mut last_pos = 0;
    let bytes = s.as_bytes();
    let len = bytes.len();
...
fn escape_vcard_iter_table(s: &str, out: &mut String) {
    let bytes = s.as_bytes();
    let mut last_pos = 0;
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("escape_vcard_methods");

    for s in &test_strings {
        group.bench_function(format!("while_matches_{}", s), |b| {
            b.iter(|| {
                let mut out = String::with_capacity(128);
                escape_vcard_while_matches(black_box(s), &mut out);
                black_box(out);
            })
        });
        ...
        group.bench_function(format!("while_table_{}", s), |b| {
            b.iter(|| {
                let mut out = String::with_capacity(128);
                escape_vcard_while_table(black_box(s), &mut out);
                black_box(out);
            })
        });
        ...
        group.bench_function(format!("iter_table_{}", s), |b| {
            b.iter(|| {
                let mut out = String::with_capacity(128);
                escape_vcard_iter_table(black_box(s), &mut out);
                black_box(out);
            })
        });
    }

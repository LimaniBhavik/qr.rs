...
fn validate_output_path(path: &Path) -> Result<(), std::io::Error> {
    ...
    if !canonical_parent.starts_with(&canonical_current) {
        return Err(std::io::Error::new(
            ...
            "Output path is outside the current working directory",
        ));
    }
            ...
            Ok(qr) => {
                ...
                if let Some(path) = output {
                    ...
                    if extension.eq_ignore_ascii_case("svg") {
                        save_svg(&generator, &path, force);
                    } else {
                        save_png(&generator, &qr, &path, logo_path, scale, border, force);
                    }
                } else {
...
fn finish_interactive_generation(builder: QRBuilder) -> Result<(), String> {
    ...
    generate(builder, path, None, None, None, false);
    Ok(())
}
